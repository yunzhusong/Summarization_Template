...
from models.adapter import insert_adapters
...
def main():
    ...
    # [For T5] need to assign prefix for summarization
    ...
    # NEW: insert adapters and load model
    if model_args.load_model_before_insert_adapter:
        ckpt_path = os.path.join(model_args.load_trained_model_from,
                                 "pytorch_model.bin")
        ckpt = torch.load(ckpt_path)
        model.load_state_dict(ckpt)

        if training_args.insert_adapters:
            model = insert_adapters(model_args, model)
    else:
        if training_args.insert_adapters:
            model = insert_adapters(model_args, model)
    ...
    if data_args.eval_dataset_name:
        train_dataset, _, _ = build_datasets(data_args,
                                             training_args,
                                             tokenizer,
                                             ignore_eval=True)
        data_args.dataset_name = data_args.eval_dataset_name
        _, eval_dataset, test_dataset = build_datasets(data_args,
                                                       training_args,
                                                       tokenizer,
                                                       ignore_train=True)
    else:
